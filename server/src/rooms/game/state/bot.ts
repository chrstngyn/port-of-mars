...
export type AbstractMarsEventVisitor = {
  [view in EventCase]: (state: GameState, player: Player) => Array<GameEvent>;
};
...
export class MarsEventVisitor implements AbstractMarsEventVisitor {
  ...
  SAVE_RESOURCE(state: GameState, player: Player): Array<GameEvent> {
  ...
  DISCARD_ACCOMPLISHMENT(state: GameState, player: Player): Array<GameEvent> {
    return [new SetPlayerReadiness({ value: true, role: player.role })];
  }
}
...
export class ActorRunner implements Actor {
  ...
  [Phase.events](state: GameState, player: Player): Array<GameEvent> {
    ...
    return this.marsEventVisitor[marsEvent.eventCaseClientHandler](
      state,
      player
    );
  }
